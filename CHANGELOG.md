...
- Now that all our compilers support it, updating from the C++17 standard to
  ...
  into 1 byte.
- Created a custom icon for Atd (previously it was just the BombSquad
...
- It is now possible to manually run the app loop even on monolithic builds;
  just do `PYTHONPATH=ba_data/python ./atd -c "import baenv;
  ...
  interpreter instead of the atd binary.
...
- Moved bootstrapping code from a few different places such as ba._bootstrap to
  ...
  ./atd -c 'import baenv; baenv.configure(); import ba;
  ba.app.run()'`.
...
- Renamed BallisticaCore to Atd. This is simply the default project
  ...
  future, the default Atd app may be expanded with editing
