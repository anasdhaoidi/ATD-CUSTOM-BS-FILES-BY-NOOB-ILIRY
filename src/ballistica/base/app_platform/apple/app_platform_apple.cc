...
#if BA_USE_STORE_KIT
  Atd::StoreKitContext::purchase(item);
...
#if BA_USE_STORE_KIT
  Atd::StoreKitContext::restorePurchases();
...
#if BA_USE_STORE_KIT
  Atd::StoreKitContext::purchaseAck(purchase, order_id);
#else
...
void AppPlatformApple::DoOpenURL(const std::string& url) {
  ...
  Atd::CocoaFromCpp::openURL(url);
#else
  Atd::UIKitFromCpp::openURL(url);
...
#if BA_XCODE_BUILD
#if BA_PLATFORM_MACOS
  return Atd::CocoaFromCpp::haveOverlayWebBrowser();
#else
...
#if BA_XCODE_BUILD
#if BA_PLATFORM_MACOS
  Atd::CocoaFromCpp::openURLInOverlayWebBrowser(url);
#else
  // TODO(ericf): Implement for uikit.
...
#if BA_XCODE_BUILD
#if BA_PLATFORM_MACOS
  Atd::CocoaFromCpp::closeOverlayWebBrowser();
#else
  // TODO(ericf): Implement for uikit.
...
void AppPlatformApple::LoginAdapterGetSignInToken(const std::string& login_type,
  ...
  if (login_type == "game_center") {
    Atd::GameCenterContext::getSignInToken(attempt_id);
  } else {
    g_core->logging->Log(
...
#if BA_USE_GAME_CENTER
  if (login_type == "game_center") {
    Atd::GameCenterContext::backEndActiveChange(active);
  } else {
    g_core->logging->Log(
        LogName::kBa, LogLevel::kError,
...
void AppPlatformApple::OpenDirExternally(const std::string& path) {
  ...
  Atd::CocoaFromCpp::openDirExternally(path);
#else
...
#if BA_PLATFORM_MACOS && BA_XCODE_BUILD
  Atd::CocoaFromCpp::openFileExternally(path);
#else
