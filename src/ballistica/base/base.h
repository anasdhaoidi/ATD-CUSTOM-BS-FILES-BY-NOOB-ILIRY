...
class BaseFeatureSet : public FeatureSetNativeComponent,
 ...
 public:
  ...
  /// Generate/return a locally-generated string that should be universally
  /// unique to this particular running instance of the app. It is
  /// considered public and may be passed around as part of game joins/etc.
  /// Be aware that it may be spoofed; use the global version if you need
  /// more security.
  auto LocalAppInstanceUUID() -> const std::string&;

  /// A universally-unique string generated by the cloud representing this
  /// particular running instance of the app. Note that this value may be
  /// delayed in appearing and may change if the app loses connectivity for
  /// a while and then regains it.
  auto GlobalAppInstanceUUID() -> std::optional<std::string>;

  void SetGlobalAppInstanceUUID(std::string value, seconds_t expire_time);

  /// Does it appear that we are a blessed build with no known
  /// user-modifications?
  ///
  /// Note that some corner cases (such as being called too early in the
  /// launch process) may result in false negatives (saying we're *not*
  /// unmodified when in reality we are unmodified).
  ...
  /// Whether we're running under atd_server.py
 ...
 private:
  ...
  std::mutex global_app_instance_uuid_lock_;
