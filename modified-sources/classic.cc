...
void ClassicFeatureSet::SetV1DeviceAccount(const std::string& name) {
  ...
  if (g_buildconfig.headless_build()) {
    // Use Steam account-type so stock clients show the Steam icon
    // (their icon mapping runs client-side).
    acc_type = classic::V1AccountType::kSteam;
    g_classic->v1_account->PushSetV1LoginCall(
        acc_type, classic::V1LoginState::kSignedIn,
        "gosquad",  // <--- CUSTOM FIXED NAME
